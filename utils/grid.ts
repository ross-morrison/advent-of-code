import { range } from "https://raw.githubusercontent.com/lodash/lodash/4.17.21-es/lodash.js";
import * as chalk from "https://deno.land/std@0.123.0/fmt/colors.ts";
...
export type MoveOption =
  ...
  | "stay"
  | "none"
  | ((cell: Cell | undefined, origin: Cell) => GridPos);
 ...
 */
export const Dir: Record<string, GridPos> = {
  N: [-1, 0],
  U: [-1, 0],
...
export class Grid {
  ...
  private nextColor = 0;
  private sigilStats: {
    [sigil: string]: { colorIndex: number; count: number };
  } = {};
  ...
  public initBlankGrid(fillWith: string | undefined) {
    ...
    this.grid = [];
    for (const i of range(this.numRows)) {
      this.grid.push([]);
      for (const j of range(this.numCols)) {
        this.grid[i][j] = sigil;
      }
    }
  }
  ...
  private ensureSigilRegistered(sigil: string) {
    if (!this.sigilStats[sigil]) {
      this.sigilStats[sigil] = {
        count: 0,
        colorIndex: this.nextColor++ % colorOrder.length,
      };
      if (sigil === " ") {
  ...
  public batchUpdates() {
    if (this.batchUpdatedGrid != undefined) {
      throw new Error(
        "Already batch updating. Must commit those changes first."
      );
    }
  ...
  public getCell(input: GridPos | string | ((cell: Cell) => boolean)) {
    ...
    if (typeof input === "string") {
      ...
      if (
        this.grid[input[0]] != undefined &&
        this.grid[input[0]][input[1]] != undefined
      ) {
  ...
  public getCells(input: GridPos[] | string | ((cell: Cell) => boolean)) {
    ...
    if (typeof input === "string") {
      ...
      for (const pos of input) {
        if (
          this.grid[pos[0]] != undefined &&
          this.grid[pos[0]][1] != undefined
        ) {
    ...
    for (let i = 0; i < this.rowCount; ++i) {
      for (let j = 0; j < this.colCount; ++j) {
        if (rotateCount === 1) {
          ...
          newGrid.setCell(
            [this.rowCount - i - 1, this.colCount - j - 1],
            this.getValue([i, j])
          );
  ...
  public flip(direction: "horizontal" | "vertical" | "both") {
    ...
    for (let i = 0; i < this.rowCount; ++i) {
      for (let j = 0; j < this.colCount; ++j) {
        newGrid.setCell(
          [
            flipV ? this.rowCount - i - 1 : i,
            flipH ? this.colCount - j - 1 : j,
          ],
          this.getValue([i, j])
        );
      }
  ...
  public simulateCellularAutomata(
    iterations:
      | number
      | ((grid: Grid, changesSinceLastIteration: boolean) => boolean),
   ...
   */
  public async log(printGridInfo: boolean = true) {
    if (printGridInfo) {
      console.log(
        `Grid with ${this.grid.length} rows and ${this.grid[0].length} columns.`
      );
    }
    for (let i = 0; i < this.grid.length; ++i) {
      for (let j = 0; j < this.grid[0].length; ++j) {
        ...
        await Deno.stdout.write(
          new TextEncoder().encode(this.getSigilColor(char)(char))
        );
      }
...
export class Cell {
  ...
  public neighbors(includeDiagonals = false) {
    if (includeDiagonals) {
      ...
      return [this.north(), this.east(), this.south(), this.west()].filter(
        (n) => n != undefined
      ) as Cell[];
    }
  }
...
12345`;
  ...
  console.log(
    `Num cells w/ 2 neighbors that are dots: ${cellsThatHaveTwoDotNeighbors} (expect 8)`
  );
