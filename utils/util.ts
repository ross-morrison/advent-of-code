import { crypto, DigestAlgorithm } from "https://deno.land/std@0.167.0/crypto/mod.ts";
...
export function formatTime(
  ms: number,
  unitsOfPrecision: number = Number.MAX_SAFE_INTEGER
) {
  const units = splitTimeIntoUnits(ms);
  const entries = Object.entries(units);
  entries.sort(
    (a, b) => TIME_UNIT_ORDER.indexOf(a[0]) - TIME_UNIT_ORDER.indexOf(b[0])
  );
  let result: string[] = [];
  const startIndex = entries.findIndex((e) => e[1] !== 0);
  for (
    let i = startIndex;
    i < Math.min(entries.length, startIndex + unitsOfPrecision);
    ++i
  ) {
    if (entries[i][0] === "ms") {
...
export function powerMod(
  _base: number | bigint,
  _exponent: number | bigint,
  _modulus: number | bigint
) {
...
export function powerSet<T>(
  inputArr: T[],
  settings: { proper?: boolean; nonEmpty?: boolean } = {}
) {
  let result = inputArr.reduce(
 ...
 *
 */
export async function getHash(str: string, method: DigestAlgorithm) {
  return await crypto.subtle.digest(method, new TextEncoder().encode(str));
}
