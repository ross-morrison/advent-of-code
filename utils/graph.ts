 ...
 * @param options
 */
export function* bfTraverse<TNode>(
  options: BFTraverseOptions<TNode>
): Generator<BFTraverseResult<TNode>> {
  function reconstructAllPaths(
    ...
    const parents =
      Array.from(options.getParents?.(node, visited) ?? []) ??
      allParents.get(node) ??
      [];
    if (parents.length === 0) {
      return [[node]];
 ...
 * @param options
 */
export function bfSearch<TNode>(
  options: BFSOptions<TNode>
): BFTraverseResult<TNode> | undefined {
  const traversal = bfTraverse({
    start: options.start,
    neighbors: options.neighbors,
 ...
 * @returns
 */
export function undirectedAllPaths<TNode>(
  options: AllPathsOptions<TNode>
): TNode[][] {
  ...
  function search(node: TNode): TNode[][] {
    ...
    for (const neighbor of neighbors) {
      if (
        !path.filter((n) => !options.canRevisit?.(n, path)).includes(neighbor)
      ) {
        path.push(neighbor);
        search(neighbor);
        path.pop();
