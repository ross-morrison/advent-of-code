...
const pq = new PriorityQueue<[number, Node, number, Set<string>]>(
  (a, b) => b[0] < a[0]
);
pq.enqueue([0, starting, 1, new Set(["AA"])]);

while (!pq.isEmpty()) {
  const [curValue, node, minute, opened] = pq.dequeue()!;
  ...
  if (!cache.has(cacheKey) || cache.get(cacheKey)! < curValue) {
    cache.set(cacheKey, curValue);
  }

  if (cache.get(cacheKey)! > curValue) {
    continue;
  }
